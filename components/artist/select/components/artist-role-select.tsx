	...
	return (
		credits?.map((item, idx) => (
				...
				>
					<DropdownMenu>
						{item.artist && (
							<DeleteItemButton onClick={() => deleteCreditItem(item.artist!)}/>
						)}
						...
						<DropdownMenuContent align="start">
							{songPreviewState?.artists?.slice(0, 4).map(artist => (
								<div
									...
									onClick={() => handleCreditsValues({ artist: artist, index: idx })}
								>
									<Typography>
										{artist.name}
									</Typography>
						...
						<DropdownMenuContent align="start">
							{roles.map(role => (
								<DropdownMenuItem
									key={role.id}
									className="flex items-center !p-2 rounded-md hover:brightness-125 cursor-pointer"
									onClick={() => handleCreditsValues({ role: role, index: idx })}
								>
									<Typography>
										{role.name}
									</Typography>
