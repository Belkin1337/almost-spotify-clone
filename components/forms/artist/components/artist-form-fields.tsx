...
export const ArtistFormFields = ({
	...
	const handleImageChange = (fieldName: keyof zodEditSchema, event: ChangeEvent<HTMLInputElement>) => {
		...
		reader.onload = () => {
			return setPreview((prev) => ({
				...prev, [fieldName]: reader.result as string
			}));
		};
	}
	
	const handleInputChange = (fieldName: keyof zodEditSchema, e: ChangeEvent<HTMLInputElement>) => {
		return setPreview(prevState => ({
	...
	return (
		<div className="flex lg:flex-row flex-col gap-x-4 gap-y-6 items-center justify-stretch">
			<div className="flex flex-col gap-y-8 w-2/3">
				...
				/>
				<Button type="submit" variant="form" align="centered" rounded="medium" background_color="default" disabled={isLoading}>
					<Typography>{uploadModalLocale('modal.submit')}</Typography>
				</Button>
			</div>
