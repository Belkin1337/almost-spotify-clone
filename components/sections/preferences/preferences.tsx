      ...
      <div className="flex flex-col items-start gap-y-4 w-full">
        <div className="flex flex-col gap-y-2 w-[60%]">
          ...
          {features(settingsLocale).map(item => (
            <div key={item.name} className="flex items-center justify-between gap-x-6 w-full bg-neutral-800 p-2 rounded-md">
              <div className="flex flex-col w-1/2">
                <Typography size="large" font="bold" text_color="white">
                  {item.attributes.name}
                </Typography>
                <Typography size="medium" text_color="gray">
